...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'jeremi-2025')
...
def on_undo(data):
    ...
    if not last.get("isDiac") and last["tile"].get("type") != "boundary":
...
def on_submit(data):
    ...
    for wt in all_words:
        ...
        if not valid:
            ...
            for pt in placed:
                if pt["tile"].get("type") != "boundary" and not pt.get("isDiac"):
...
def _next_turn(room_id):
    ...
    socketio.emit("your_turn", {"player": np["name"], "player_index": room["current_turn"]}, room=room_id)
...
def _end_game(room_id, reason):
    ...
    socketio.emit("game_over", {
        "reason": reason, "final_scores": scores,
        "winner": scores[0]["name"], "deductions": deductions
    }, room=room_id)
